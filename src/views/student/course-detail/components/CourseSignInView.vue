...
import { getSign, sign } from '@/api/courseApi.js'
...
const openSignModal = attendance => {
  // 查找对应的签到记录
  const attendanceItem = signInList.value.find(
    item => item.attendanceId === attendance,
  )
  if (attendanceItem) {
...
<template>
  <div class="app p-4">
    <h2 class="text-2xl font-bold mb-6">课程签到</h2>
    ...
    <!-- 空状态 -->
    <div v-else-if="signInList.length === 0" class="card">
      <div class="card-body text-center">
        <h3 class="text-lg font-medium">暂无签到记录</h3>
        <p class="text-base-content/70">当前课程还没有发起过签到</p>
      </div>
    </div>
        ...
        <div class="card-body">
          ...
          <div class="text-sm text-base-content/70 mt-2">
            <div class="flex flex-col gap-1">
              <p>
                创建时间: {{ formatDateObject(item.createdAt).dateTime }}
                {{ formatDateObject(item.createdAt).weekDay }}
              </p>
              <p v-if="item.pinCode">签到码: {{ item.pinCode }}</p>
            </div>
          </div>
    ...
    <dialog :class="{ modal: true, 'modal-open': isModalOpen }">
      <div class="modal-box">
        <h3 class="font-bold text-lg mb-4">课程签到</h3>

        <div v-if="currentAttendance" class="py-4">
          <!-- PIN码签到 -->
          <div
        ...
        <div class="modal-action">
          ...
          <button
            ...
            @click="submitSignIn"
          >
            <span v-if="signingIn" class="loading loading-spinner" />
            确认签到
          </button>
        </div>
      </div>
