  ...
  <div class="app p-4 w-full h-screen overflow-y-auto overflow-x-hidden">
    ...
    >
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-2xl font-bold mb-2">今日教学概览</h2>
          <p class="text-gray-600">继续为学生提供优质的教学体验！</p>
        </div>
      </div>
    </div>
    ...
    >
      ...
      >
        ...
        >
          <div class="stat-figure text-primary">
            <BookOne theme="outline" size="32" />
          </div>
          <div class="stat-title">所教课程</div>
          <div class="stat-value text-primary">
            {{ teacherStats.courseCount }}
          </div>
          <div class="stat-desc">当前学期</div>
        </div>
      </div>
      <!-- 学生总数 -->
      <div
      ...
      >
        <div class="stat-figure text-accent">
          <ExamineFile theme="outline" size="32" />
        </div>
        <div class="stat-title">平均成绩</div>
        <div class="stat-value text-accent">
          {{ teacherStats.avgExamScore }}
        </div>
        <div class="stat-desc">最近一次测验</div>
      </div>
    ...
    >
      <h3 class="text-xl font-bold mb-4">教学建议</h3>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="flex items-start space-x-3">
          <div class="bg-primary/10 p-2 rounded-full">
            <Time theme="outline" size="24" class="text-primary" />
          </div>
          <div>
            <h4 class="font-semibold">教学时长分析</h4>
            <p class="text-sm text-gray-600">
              本周授课{{ teacherStats.weeklyTeachingHours }}小时，比上周增加了{{
                teacherStats.weeklyTeachingHours -
                teacherStats.lastWeekTeachingHours
              }}小时
            </p>
          </div>
        </div>
        <div class="flex items-start space-x-3">
          <div class="bg-secondary/10 p-2 rounded-full">
            <ExamineFile theme="outline" size="24" class="text-secondary" />
          </div>
          <div>
            <h4 class="font-semibold">成绩分布建议</h4>
            <p class="text-sm text-gray-600">
